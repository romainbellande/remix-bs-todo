...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-blues-stack-6aed
  fly apps create remix-blues-stack-6aed-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-blues-stack-6aed
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-blues-stack-6aed-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-blues-stack-6aed-staging' does not match app name in config file 'remix-blues-stack-6aed'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-blues-stack-6aed-db
  fly postgres attach --app remix-blues-stack-6aed remix-blues-stack-6aed-db

  fly postgres create --name remix-blues-stack-6aed-staging-db
  fly postgres attach --app remix-blues-stack-6aed-staging remix-blues-stack-6aed-staging-db
  ```
